class Query():
    ...
    SELECT_A = """(select * from review join customer on rev_customer_id = customer_id where rev_pro_id = %d order by rev_rating desc, rev_helpful desc limit 5) union all (select * from review join customer on rev_customer_id = customer_id where rev_pro_id = %d order by rev_rating, rev_helpful desc limit 5);""" 
    SELECT_B = """select p2.pro_id,p2.pro_asin,p2.pro_title,p2.pro_salesrank from product p, similarproducts, product p2 where p.pro_asin = sim_pro_asin and p2.pro_asin = sim_pro_sim_asin and p.pro_salesrank <= p2.pro_salesrank and p.pro_id = %d""" 
    SELECT_C = """select r.rev_date, avg(rev_rating) as rev_rating_average from product join review r on rev_pro_id = pro_id where pro_id = %d group by r.rev_date order by rev_date ;"""
